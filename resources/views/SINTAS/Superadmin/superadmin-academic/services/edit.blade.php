    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Edit Service: :name', ['name' => $service->name]) }}
            </h2>
            ...
            <div class="bg-white overflow-hidden shadow-lg border border-gray-200 sm:rounded-2xl">
                <div class="p-8">
                    <form method="POST" action="{{ route('admin.services.update', $service) }}" class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="class_mode" id="class_mode"
                                        ...
                                        <option value="online" {{ old('class_mode', $service->class_mode) == 'online' ? 'selected' : '' }}>Online</option>
                                        <option value="offline" {{ old('class_mode', $service->class_mode) == 'offline' ? 'selected' : '' }}>Offline</option>
                                        <option value="both" {{ old('class_mode', $service->class_mode) == 'both' ? 'selected' : '' }}>Both</option>
                                    </select>
                                    @error('class_mode')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="text" name="features" id="features" value="{{ old('features', $service->features) }}"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="number" name="min_age" id="min_age" value="{{ old('min_age', $service->min_age) }}" min="0"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="max_age" id="max_age" value="{{ old('max_age', $service->max_age) }}" min="0"
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center">
                                    ...
                                    <input type="checkbox" name="for_parent_register" id="for_parent_register" value="1" {{ old('for_parent_register', $service->for_parent_register) ? 'checked' : '' }}
                                ...
                                <div class="flex items-center">
                                    ...
                                    <input type="checkbox" name="for_self_register" id="for_self_register" value="1" {{ old('for_self_register', $service->for_self_register) ? 'checked' : '' }}
                                           class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        ...
                        <!-- Service Statistics -->
                        @if($service->programs_count > 0)
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
                            <h4 class="text-sm font-semibold text-blue-800 mb-3">Service Statistics</h4>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                <div>
                                    <span class="text-blue-600">Programs:</span>
                                    <span class="font-semibold text-blue-800 ml-1">{{ $service->programs_count }}</span>
                                </div>
                                <div>
                                    <span class="text-blue-600">Active Programs:</span>
                                    <span class="font-semibold text-blue-800 ml-1">{{ $service->programs()->active()->count() }}</span>
                                </div>
                                <div>
                                    <span class="text-blue-600">Class Mode:</span>
                                    <span class="font-semibold text-blue-800 ml-1">{{ ucfirst($service->class_mode) }}</span>
                                </div>
                                <div>
                                    <span class="text-blue-600">Created:</span>
                                    <span class="font-semibold text-blue-800 ml-1">{{ $service->created_at->format('M d, Y') }}</span>
                                </div>
                            </div>
                        </div>
                        @endif
