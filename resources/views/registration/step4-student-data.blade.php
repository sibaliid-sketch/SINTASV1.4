<x-app-layout>
        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex-1 h-2 bg-gradient-to-r from-primary-600 to-secondary-600 rounded-full" style="width: 28%"></div>
                    <div class="flex-1 ml-2 h-2 bg-gray-300 rounded-full" style="width: 72%"></div>
                </div>
                <div class="text-right text-sm font-semibold text-gray-600">Section 3 dari 9</div>
            </div>
            ...
            <div class="bg-white/80 backdrop-blur-md overflow-hidden shadow-lg border border-gray-200/50 sm:rounded-2xl">
                <div class="py-4 px-6 text-center">
                    ...
                    <form method="POST" action="{{ route('registration.step4-submit') }}" class="space-y-6">
                        ...
                        <input type="hidden" name="education_level" value="{{ session('education_level') }}">
                        <input type="hidden" name="class_level" value="{{ session('class_level') }}">

                        <!-- Data Siswa Section -->
                        <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                            <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">📋 @if(session('is_parent_register') === 'yes') Masukkan Data Anak Anda @else Masukkan Data diri Anda @endif</h4>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nama Lengkap @if(session('is_parent_register') === 'yes') Anak @endif *</label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">@if(session('is_parent_register') === 'yes') Nomor Whatsapp Anak @else Nomor Whatsapp @endif</label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">@if(session('is_parent_register') === 'yes') Email Anak @else Email @endif</label>
                        ...
                        <div id="parent-data-section" class="border-t border-gray-200 dark:border-gray-700 pt-6 @if(session('is_parent_register') !== 'yes') hidden @endif">
                        ...
                        <div class="flex gap-4 pt-6 border-t border-gray-200">
                            <a href="{{ route('registration.step2') }}" class="px-6 py-3 bg-gray-200 text-gray-800 rounded-xl hover:bg-gray-300 transition-all duration-200 font-medium">
                                ← Kembali
                            </a>
    ...
    <script>
        ...
        const isParentRegister = '{{ session("is_parent_register") }}';

        function checkAgeAndShowParentData() {
            if (isParentRegister === 'no' && birthdateInput.value) {
                ...
                if (age < 18) {
                    ...
                    const parentInputs = parentDataSection.querySelectorAll('input[required], select[required], textarea[required]');
                    ...
                    const parentInputs = parentDataSection.querySelectorAll('input, select, textarea');
                    parentInputs.forEach(input => {
        ...
        birthdateInput.addEventListener('change', checkAgeAndShowParentData);
        // Check on load
        checkAgeAndShowParentData();
    </script>
</x-app-layout>
